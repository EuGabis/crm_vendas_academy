...
import {
  getAccumulatedPointsGoalToToday,
  getAccumulatedPointsVsGoal,
  getConsolidatedGoal,
  getCoursesSold,
...
export function DashboardTimes() {
  ...
  const data = useMemo(() => {
    ...
    const accumulatedGoal = getAccumulatedPointsGoalToToday(ds, filter);
    const projection = getPointsProjection(ds, filter);
    const accVsGoal = getAccumulatedPointsVsGoal(ds, filter);
    const breakdown = getPaymentMethodBreakdown(ds, filter);
    const cards = getSellerCards(ds, filter);
    const ranking = [...cards].sort((a, b) => b.points - a.points);
    return {
      goal,
      revenue,
      points,
  ...
  const pace = data.accumulatedGoal ? data.points / data.accumulatedGoal : 0;
  const paceAccent = pace >= 1 ? 'success' : pace >= 0.85 ? 'warning' : 'danger';
  const goalProgress = data.goal.pointsGoal ? data.points / data.goal.pointsGoal : 0;
        ...
        {ds.isLoading ? (
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
              <KpiCard
                label="Meta do mês"
                value={pts(data.goal.pointsGoal)}
              ...
              <KpiCard
                ...
                value={pts(data.accumulatedGoal)}
              ...
              <KpiCard
                ...
                value={pts(data.projection)}
                icon={<TrendingUp className="h-4 w-4" />}
                hint="Ritmo atual × dias úteis"
                accent={data.projection >= data.goal.pointsGoal ? 'success' : 'warning'}
              />
            </div>
            ...
            <Card>
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-zinc-500">
                    {pts(data.points)} de {pts(data.goal.pointsGoal)}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between mt-3 text-xs">
                <span className="text-zinc-500">
                  Acumulado até hoje: {pts(data.accumulatedGoal)}
                </span>
                {pace < 1 && data.accumulatedGoal > 0 && (
                  <span className="inline-flex items-center gap-1 text-amber-400">
                    <AlertCircle className="h-3 w-3" />
                    Faltam {pts(data.accumulatedGoal - data.points)} para o pace ideal
                  </span>
                )}
                {pace >= 1 && data.accumulatedGoal > 0 && (
            ...
            <Card>
              <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                <div>
                  <h3 className="text-sm font-semibold text-white">Pontos vs Meta — acumulado</h3>
                  <p className="text-xs text-zinc-500">
                    Realizado até hoje · projeção até o fim do mês
                    {isIndividual && sellerInfo ? ` · ${sellerInfo.fullName}` : ' · time todo'}
                  </p>
                </div>
                {data.accumulatedGoal > 0 && (() => {
                  const deltaPct = pace - 1;
                  const isAhead = deltaPct >= 0;
                  return (
                    <Badge
                      variant={isAhead ? 'success' : deltaPct >= -0.15 ? 'warning' : 'danger'}
                      className="text-xs"
                    >
                      {isAhead ? '▲' : '▼'} {formatPercent(Math.abs(deltaPct), 1)}{' '}
                      {isAhead ? 'à frente da meta' : 'atrás da meta'}
                    </Badge>
                  );
                })()}
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {data.ranking.map((row, idx) => {
                    const pct = row.pointsGoal ? row.points / row.pointsGoal : 0;
                    const accent =
                      ...
                      <Card key={row.seller.id} className="!p-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center justify-between text-[11px]">
                            <span className="text-zinc-500">Meta {pts(row.pointsGoal)}</span>
