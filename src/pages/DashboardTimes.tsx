...
export function DashboardTimes() {
  ...
  const data = useMemo(() => {
    const goal = getConsolidatedGoal(filter);
    const revenue = getRevenueSum(filter);
    const courses = getCoursesSold(filter);
    const accumulatedGoal = getAccumulatedGoalToToday(filter);
    const projection = getProjection(filter);
    const accVsGoal = getAccumulatedVsGoal(filter);
    const breakdown = getPaymentMethodBreakdown(filter);
    const cards = getSellerCards(filter);
  ...
  const sellerInfo = isIndividual ? sellers.find((s) => s.id === sellerId) : null;
  ...
  return (
    <>
      <Header
        title={isIndividual ? `Dashboard — ${sellerInfo?.fullName}` : 'Dashboard Times'}
        subtitle={
          isIndividual
      ...
      <div className="p-8 space-y-6">
        ...
        {/* Goal progress bar (consolidated) */}
        <Card>
          ...
          <div className="flex items-center justify-between mt-3 text-xs">
            <span className="text-zinc-500">Acumulado de hoje: {formatCurrency(data.accumulatedGoal)}</span>
            {pace < 1 && (
            ...
            {pace >= 1 && <span className="text-emerald-400">Acima do pace ideal ✓</span>}
          </div>
        </Card>

        {/* Main chart: cumulative sold vs goal */}
        <Card>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-zinc-500">{formatInt(data.courses)} vendas no mês</span>
          </div>
        ...
        {/* Sellers grid — visible only in consolidated mode */}
        {!isIndividual && (
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-white">Vendedores</h3>
              <span className="text-xs text-zinc-500">Clique para filtrar visão individual</span>
            </div>
                ...
                return (
                  <Card key={row.seller.id} className="!p-5">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Badge
                        variant={idx === 0 ? 'success' : idx <= 2 ? 'default' : 'muted'}
                      >
                        #{idx + 1}
                      </Badge>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-zinc-800/80 grid grid-cols-3 gap-2 text-center">
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-500">Cursos</p>
                        <p className="text-sm font-semibold text-zinc-200">{row.coursesSold}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-500">Leads</p>
                        <p className="text-sm font-semibold text-zinc-200">{row.leadsCount}</p>
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-500">Conv.</p>
