...
export function FunilVendas() {
  ...
  const funnel = useMemo(() => getFunnelMetrics(filter), [year, month, sellerId]);
  const sellerCards = useMemo(() => getSellerCards(filter), [year, month, sellerId]);
  ...
  const noShows = getStageCount(filter, 'NO_SHOW');
    ...
    <>
      ...
      <div className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-zinc-500">Evolução do mês — total: {formatInt(funnel.total)}</p>
              </div>
            </div>
            <div className="h-72">
          ...
          {/* No-Shows card — réplica do mockup */}
          <Card>
                  ...
                  <tbody className="divide-y divide-zinc-900">
                    {filteredSellers.map((row) => (
                      <tr key={row.seller.id} className="hover:bg-zinc-900/40 transition-colors">
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="leading-tight">
                              ...
                              <div className="text-[10px] text-zinc-500">{row.seller.team}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="text-right tabular-nums text-zinc-300">{row.agendadas}</td>
                        <td className="text-right tabular-nums text-zinc-300">{row.realizadas}</td>
                        <td className="text-right tabular-nums text-amber-300">{row.noShows}</td>
