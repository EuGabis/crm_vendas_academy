...
import { getMarketingMetrics, getSpendByDay, getMonthTrafficSpend } from '@/data/queries';
...
export function Marketing() {
  ...
  const metrics = useMemo(() => getMarketingMetrics(filter), [year, month]);
  const dailySpend = useMemo(() => getSpendByDay(filter), [year, month]);
  const monthSpend = useMemo(() => getMonthTrafficSpend(filter), [year, month]);
  ...
  const cacWarning = metrics.cac > 0.3 * metrics.ticketMedio;
